...
1. Load target features from `.agents/feature-contexts/*.json` (or a specific id).
...
- upsert entry into `.agents/feature-history.json`
- remove `.agents/feature-contexts/<id>.json`
- remove worktree paths where `worktree: true` (best effort)
- update `.agents/current-feature` if needed
